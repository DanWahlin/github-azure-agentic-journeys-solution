...
Call external tools with `execFileSync()` or `spawnSync()` and argument arrays. Do not concatenate a shell command, use `chmod`, or depend on Bash, `cut`, `grep`, or `date`. The static Windows PowerShell launcher described above is the only platform-specific exception; all CLI arguments must travel in the JSON environment payload. A Windows `.cmd` or `.bat` shim cannot losslessly carry an argument containing a literal double quote, so the launcher must reject that case before invocation. Rewrite such CLI arguments without embedded double quotes; native `.exe` targets preserve them. Use JavaScript for path handling, timestamps, retries, and JSON parsing.
