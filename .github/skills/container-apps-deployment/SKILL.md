...
Call external tools with `execFileSync()` or `spawnSync()` and argument arrays. Do not concatenate a shell command, use `chmod`, or depend on Bash, `cut`, `grep`, or `date`. The static Windows PowerShell launcher described above is the only platform-specific exception; all CLI arguments must travel in the JSON environment payload. A Windows `.cmd` or `.bat` shim cannot losslessly carry shell metacharacters (`"`, `&`, `|`, `<`, `>`, `^`, `%`, `!`, `(`, `)`) or CR/LF inside an argument, so the launcher must reject those cases before invocation. Rewrite such CLI arguments without metacharacters; native `.exe` targets preserve them. Use JavaScript for path handling, timestamps, retries, and JSON parsing.
