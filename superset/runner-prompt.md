...
Work only inside this superset folder. Generate all infrastructure, manifests, hooks, reports, logs, and screenshots here. Host is Linux ARM64; required host tools have passed preflight. Use only portable CommonJS .js or .ts host hooks and argument arrays.

Deploy with subscription [REDACTED], location westus, azd environment rr-superset-0717, and resource group rg-rr-superset-0717. Use AKS, not Container Apps. Generate secure credentials without printing them. Generate infra-superset/hooks/postprovision.js as CommonJS to get AKS credentials, invoke Helm 3 and kubectl with argument arrays, apply manifests, and poll the LoadBalancer. Configure the AKS pool for a region without availability zones, use PostgreSQL rather than SQLite, and implement the shared emptyDir psycopg2 pattern.
